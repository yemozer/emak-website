<template>
  <section id="brand-partners" class="py-8 md:py-10 lg:py-12 bg-white border-b border-[rgb(178,178,178)]/10">
    <div class="container mx-auto px-4 lg:px-8">
      ...
      <div class="grid grid-cols-5 gap-2 md:gap-3 lg:gap-4">
        <div
          v-for="brand in brands"
          :key="brand.name"
          class="group flex items-center justify-center rounded-lg md:rounded-lg border border-[rgb(178,178,178)]/20 bg-white p-2 md:p-3.5 lg:p-4.5 shadow-sm transition-all hover:shadow-md hover:scale-105 hover:border-[rgb(39,45,122)]/30 active:scale-95"
        >
          <img
            v-if="brand.logo"
            :src="brand.logo"
            :alt="brand.name"
            class="h-6 md:h-7 lg:h-9 w-auto max-w-full object-contain opacity-90 transition-opacity group-hover:opacity-100"
            @error="handleImageError"
          />
          <span v-else class="text-center text-[9px] md:text-[10px] lg:text-xs font-semibold text-[rgb(39,45,122)] leading-tight px-0.5">{{ brand.name }}</span>
        </div>
      </div>
    </div>
  </section>
...
const brands = [
  ...
  { name: 'BONUS YALITIM', logo: '/brands/bonus-logo.png' },
  ...
  { name: 'BTG GAZBETON', logo: '/brands/btg-logo.png' },
  { name: 'ENTEGRE', logo: '/brands/entegre-logo.png' },
  { name: 'FİXA', logo: '/brands/fixa-logo.png' },
  { name: 'KALDE', logo: '/brands/kalde-logo.png' },
  ...
  { name: 'TEKNOBİMS', logo: '/brands/teknobims-logo.png' },
